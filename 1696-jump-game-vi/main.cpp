...
public:
	int maxResult(const std::vector<int>& nums, int k) {
		...
		_len = nums.size();
		if (_len == 1) { return nums.at(0); }
		_k = (size_t)k;

		// Edge case: what if k >= _len-1?
		// In that case, nums[1] to nums[_len-1] can be skipped at anytime.
		//
		if (_k >= _len-1) {

			int result{nums.at(0)+nums.at(_len-1)};
			for (size_t ii{1}; ii<_len-1; ++ii) {
				if (nums.at(ii) > 0) { result += nums.at(ii); }
			}
			return result;
		}
		...
		// Gap strategy, where "gaps" are where negative numbers are located
		//
		std::list<std::pair<size_t,size_t>> gaps; // pair<beginning of a gap, size of said gap> from 1 to _len-1
												  // e.g., {1,-2,-3,-4,5,-6,-7,8} -> {{1,3},{5,2}}
		size_t gapStart{0}, gapSize{0};
		for (size_t ii{1}; ii < _len-1; ++ii) {

			if (_nums.at(ii) >= 0 && gapSize != 0) {

				gaps.push_back({gapStart,gapSize});
				gapStart = ii;
				gapSize = 0;

			} else if (_nums.at(ii) < 0) {
		...
		// Taking the gaps into consideration...
		int result{_nums[0] + _nums[_len-1]};
		size_t prev{1}; // _nums[prev:gap.first] is all positive
		for (const auto& gap : gaps) {
			result += sum(prev, gap.first);
			prev = gap.first+gap.second;

			if (gap.second >= _k) {
				result += gapThrough(gap.first, gap.second);
			}
		}
		result += sum(prev, _len-1);
		return result;
	}

private:

	// sum of _nums[ii] where from<=ii<to
	int sum(size_t from, size_t to) {
	...
	// gapThrough goes through the "gap" identified as _nums[ii] where from<=ii<=from+width
	// (i.e., width is how wide the gap is) and returns the highest possible sum.
	//
	int gapThrough(size_t from, size_t width) {

		// Now *this* is where you'd use dynamic programming.
		//
		std::vector<int> accumd(width+_k-1, 0); // accumd[ii] = _nums[from+ii]+max(accumd[ii+1],accumd[ii+2],...,accumd[ii+k]);
		accumd[width-1] = _nums[from+width-1];
		for (size_t ii{width-2}; ii<width; --ii) {
			int record{_nums[from+ii]+accumd[ii+1]};
			for (size_t jj{2}; jj<=_k; ++jj) {
				record = std::max(record, _nums[from+ii]+accumd[ii+jj]);
			}
			accumd[ii] = record;
		}
		...
		//
		int record{accumd[0]};
		for (size_t ii = 1; ii < _k; ++ii) {
			record = std::max(record, accumd[ii]);
		}
		return record;
	}
};
...
struct Testcase{
	...
	const int _k;
	const int _expected;
	Testcase(const std::vector<int> nums, int k, int expected)
		: _nums{nums}, _k{k}, _expected{expected} {}

	bool test(Solution& soln) const { 
		int result{soln.maxResult(_nums, _k)};
		std::cout << "Expected: " << _expected << "; Actual: " << result << std::endl;
		return result == _expected;
	}
	std::string details() const {

		std::string result{"Testcase: _nums:{"};
		for (const int& num : _nums) {
			result += std::to_string(num) + " ";
		}
		result += "}; _k: " + std::to_string(_k) + "; _expected: " + std::to_string(_expected);
		return result;
	}
};
